...
/**
 * The setter computes the actual Mandel/Julia Set. 
...
public final class MandelSetter {
    ...
    /*
     * Resets the set to starting coordinates, incl. zoom value.
     * Colors and modes etc. remain the same.
     */
    public void reload() {
     ...
     * Resizes an array.
     * 
     * atm: complete recalculation. To be optimized.
     */
     ...
     * 
     * TODO: Optimize by calculating only parts that were previously unseen.
     */
    ...
    private void calculate() {
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            @Override
            public void run(){
                /* the Progress Bar is only visible during calculation */
                ProgBar.setVisible(true);
        
                maxIteration = 255 + 2*(int)(zoom);  /* Setting the maximum iteration */
                mandelSet[width][0] = maxIteration;  /* and saving it for further usage */

                realWidth = Math.abs(xStart-xEnd);   /* getting the actual width (as in coordinates), */
                realHeight = Math.abs(yStart-yEnd);  /* height and increment */
                increment = getIncrement();

                /*circle through every pixel needed */
                for (yIter = 0; yIter < height; yIter++){
                    ...
                    for (xIter = 0; xIter < width; xIter++) {
                        ...
                        while ((iteration < maxIteration) && (done == false)) {
                            ...
                            /* quick and dirty for the classic of 2 */
                            if (mode ==2) {
                                double temp = xPara;
                                xPara = xPara * xPara - yPara * yPara + x;
                                yPara = 2 * temp * yPara +y;
                            
                            /* correctly and slow for everything else */
                            } else {
    ...
    /*
     * This needs to go into its own class, alongside with the Pascal Triangle
     */
